#!/usr/bin/env python3
"""
ollama-fsb (sandboxed variant of ollama-fs)

Confines a local Ollama model to a single workspace directory. The model
can read, write, move, and delete files relative to that workspace; every
path is checked against the boundary and traversal attempts are blocked.
There is no shell-exec tool - that would defeat the sandbox.

Sibling: ollama-open.py (unsandboxed: full user-level FS + shell access).
"""
...
parser = argparse.ArgumentParser(
    ...
    epilog=(
        'Examples:\n'
        '  ollama-fsb.py -m gemma4 ./my_project\n'
        '  ollama-fsb.py -m gemma4 -a openclaw ./my_project\n'
        '  ollama-fsb.py -m gemma4 ./my_project --safe\n'
        '  ollama-fsb.py -m gemma4 ./my_project --safe --debug\n'
        '  ollama-fsb.py -a openclaw ./my_project          # agent only; model inferred from agent\n'
    )
)
...
tools = [
    ...
    },
    {
        ...
        'function': {
            ...
            'description': (
                'Write text content to a file inside the workspace. Use mode="overwrite" '
                '(default) to replace contents, or mode="append" to add to the end of an '
                'existing file (creates it if absent). Parent directories are created '
                'automatically. Use append mode when accumulating output across multiple '
                'calls, such as building up a report or log file incrementally.'
            ),
            'parameters': {
                'type': 'object',
                'properties': {
                    ...
                    'content': {
                        'type': 'string',
                        'description': 'The exact text content to write or append'
                    },
                    'mode': {
                        'type': 'string',
...
def read_local_file(relative_path: str) -> str:
    ...
    try:
        ...
        return f"Error: Permission denied reading '{relative_path}'."
    except FileNotFoundError:
        return f"Error: File not found: '{relative_path}'."
    except UnicodeDecodeError:
        return f"Error: '{relative_path}' is not a UTF-8 text file (binary?)."
    except Exception as e:
        return f"Error reading file: {str(e)}"


def write_local_file(relative_path: str, content: str, mode: str = 'overwrite') -> str:
    """
    Write content to a file inside the sandbox.
    mode='overwrite' (default) replaces contents; mode='append' adds to the end.
    In --safe mode, shows a unified diff (overwrite) or content preview (append)
    and asks the user to confirm before committing.
    """
    safe_path = _safe_resolve(relative_path)
    if safe_path is None:
    ...
    if SAFE_MODE:
        if append_mode:
            existing_size = os.path.getsize(safe_path) if os.path.exists(safe_path) else 0
            print(f"\n[Safe Mode] Append to '{relative_path}'  (existing size: {existing_size} bytes)")
        ...
        while True:
            ...
            if answer in ('y', 'yes'):
                ...
                debug_log('write_denied', {'path': relative_path, 'mode': mode})
                return f"{action_word.capitalize()} cancelled by user for '{relative_path}'."
            else:
    ...
    try:
        ...
        with open(safe_path, open_mode, encoding='utf-8') as f:
            f.write(content)
        debug_log('tool_call', {'op': 'write', 'path': relative_path, 'mode': mode, 'bytes': len(content)})
        if append_mode:
            return f"Success: Appended {len(content)} bytes to {relative_path}"
        return f"Success: Written to {relative_path}"
    except PermissionError:
        return f"Error: Permission denied writing '{relative_path}'."
    except Exception as e:
...
messages = [{
    ...
    'content': (
        ...
        f'paraphrase, or invent tool output. If the user asks you to list, show, '
        f'or display something, reproduce the actual tool output - never make up '
        f'filenames or content based on what you "expect" the workspace to '
        f'contain. If you got data back, that is the real data.\n'
        f'\n'
            ...
            if tool_calls:
                ...
                for tool in tool_calls:
                    ...
                    if func_name == 'list_directory_contents':
                        ...
                        result = write_local_file(rel_path, args_['content'], args_.get('mode', 'overwrite'))
